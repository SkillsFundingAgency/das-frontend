...
(function (Modules) {
    ...
    Modules.AppStepNav = function () {
        ...
        this.start = function ($element) {
            ...
            bindToggleForSteps(_stepNavTracker)
            bindToggleShowHideAllButton(_stepNavTracker)
            ...
            <button aria-expanded="false" class="app-step-nav__button app-step-nav__button--controls js-step-controls-button">${actions.showAllText}</button></div>`)
            }
            ...
            function showPreviouslyOpenedSteps() {
                ...
                if (data.length > 0) {
                    $showOrHideAllButton.attr(ariaExpanded, true)
                    setShowHideAllText()
                }
            }
            ...
            function ensureOnlyOneActiveLink() {
                ...
                var lastClicked = loadFromSessionStorage(sessionStoreLink) || $element.find('.' + activeLinkClass).first().attr(dataPosition)
                ...
                if (!$element.find(`.js-link[data-position="${lastClicked}"]`).parent().hasClass(activeLinkClass)) {
                    lastClicked = $element.find('.' + activeLinkClass).first().find('.js-link').attr(dataPosition)
                }
            ...
            function bindToggleShowHideAllButton(stepNavTracker) {
                ...
                $showOrHideAllButton.on('click', function () {
                    ...
                    $showOrHideAllButton.attr(ariaExpanded, shouldshowAll)
                    setShowHideAllText()
        ...
        function StepView($stepElement) {
            ...
            function isShown() {
                return $stepElement.hasClass(stepIsShown)
            }
        ...
        function StepNavTracker(totalSteps, totalLinks, iuniqueId) {
